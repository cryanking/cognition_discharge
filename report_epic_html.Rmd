...
header-includes:
    - \usepackage{colortbl}
---
<!-- docker run -v '/mnt/ris/Active/ActFastData/:/research/'  -v '/home/cognition_discharge/gitdirs/cognition_discharge/:/code/' cryanking/cognitioncheck:1.1 R --args commit=$(git --git-dir /code/.git rev-parse --verify HEAD) -e 'rmarkdown::render("/code/report_epic.Rmd")'
...
ggsave(filename="/research/testing_ggp.pdf", plot=sp, device="pdf", width=4, height=4, units="in")
...
## Results
The filtered dataset included `r merged_data2 %>% pull("CSN") %>% n_distinct` distinct patients with `r merged_data2 %>% pull("orlogid") %>% n_distinct` hospitalizations with inclusion filtering illustrated in Table \@ref(tab:fig1). 

```{r tableonea, echo=FALSE, messages=FALSE}
figure1[,c("Stage", "N")] %>% kbl(caption="\\label{tab:fig1}Patient inclusion flow") %>% 
...
knitr::include_graphics("/research/testing_ggp.pdf")
```

## Combining Datasets
As shown in Figure \@ref(fig:fig1), the Pre-Epic and Epic datasets were fairly consistent in their association between impaired cogition and discharge to other than home. A fixed-effects combination of the two estimates yielded an odds-ratio of `r exp(cis_inter_year2[nrow(cis_inter_year2), "value" ])%>% round(2) %>% sprintf(fmt="%.2f")`  , 95\\% CI (`r exp(cis_inter_year2[nrow(cis_inter_year2), c("vlow", "vhigh") ]) %>% round(2) %>% sprintf(fmt="%.2f") %>% paste(collapse=" to ")` ).
