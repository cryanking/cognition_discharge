...
bind_rows(N_mv_surg, N_epic_surg) %>% group_by(`Surgery Type`) %>% summarize(N=sum(N)) -> N_surg
...
factor_vars<- c('Sex' , 'RACE' , "CAD", "CHF", "Diabetes", "COPD", "CKD" , "CVA(TIA)", "HTN",  "cancerStatus", "AFIB", "low_functional_capacity", "low_barthel") 
...
merged_data2 %>% select(-c( "ueavfist_codes")) %>% summarize(across(ends_with("codes") , sum, na.rm=TRUE ) ) %>% t %>% as_tibble(rownames="SurgeryType") %>% swap_pretty_names %>%  set_colnames(c('Surgery Type', 'N')) %>% arrange(N)%>% kbl( caption="\\label{tab:descType}Number of Procedures by Type")
```
...
Results from logistic regression models predicting pneumonia (PNA), incident Atrial Fibrilation (AF), AKI worse than grade 1, incident stroke or TIA (CVA), ICU, elevated postop troponin, and intubation longer than 12 hours are given in Table \@ref(tab:tablethree). 
```{r tablethree, echo=FALSE, messages=FALSE, results='asis'}
exploratory_outcomes_glm  %>% kbl( caption="\\label{tab:GLM2}GLM results for exploratory outcomes. Odds ratios, 95\\% confidence intervals, and p-values.") %>%   kable_styling()
```
