...
## for xgb variables that are binary + missing can be treated as numeric
...
## the cleverness with cumsum isn't exactly what I wanted, just write a loop
...
cv_results_mv <- xgboost_cv(label=(hosp_proc$dc_status!="home" )%>% as.numeric, data=temp %>% xgb.DMatrix , sobol_size=45, nrounds=50, eval_metric="auc" )

whole_data_classifier <- cv_results_mv[[3]]
...
temp <- train_set %>% mutate(AD8=as.numeric(events_rates[i,"AD8"] ), SBT = as.numeric(events_rates[i,"SBT"] ) ) %>% mutate_all(as.numeric) %>% as.matrix
temp[!is.finite(temp)] <- NA
events_rates$imputed_rate[i] <- mean(predict(whole_data_classifier, temp))
}
...
# round((numerator %>% select(-AD8) %>% as.matrix %>% single_thresh_sum) / (denominator %>% select(-AD8)%>% as.matrix %>% single_thresh_sum), 2)  %>% as_tibble %>% mutate(AD8=c(NA,0, seq.int(3))) %>% relocate(AD8) %>% filter(is.finite(AD8)) %>% select(-SBTNA) %>% write_csv("/research/outputs/sup_nonlinear_marginal_effect.csv") 
...
train_set <- merged_data2 %>% select(one_of(comborbid_vars, surg_vars, year_vars, "AD8", "SBT", "age") )

temp <- train_set %>% mutate_all(as.numeric) %>% as.matrix
temp[!is.finite(temp)] <- NA

cv_results_mv <- xgboost_cv(label=(merged_data2$dc_home )%>% as.numeric, data=temp %>% xgb.DMatrix , sobol_size=45, nrounds=50, eval_metric="auc" )
...
bootfunction <- function(indicies, data, labels) {
  local_model <- xgb.train( data[indicies, ] , labels[indicies], params= whole_model_params[1] )
  results <- predict(local_model , data) 
  return(results)
}
