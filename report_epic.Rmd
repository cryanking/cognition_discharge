...
We found that the Vuong test of model superiority for predicting discharge location with AD8 vs SBT was `r if_else(analysis_pipe_vu_output$p_LRT %>% unlist %>% min %>% is_less_than(.05), "" ,  "not ")` significant with the minimum p value ` r print(analysis_pipe_vu_output$p_LRT%>% unlist %>% min )`. 
