...
We found that impaired cognition was `r if_else(coef_home[3] < .05,"".  "not ")` significantly associated with discharge to home (odds ratio `r paste0(exp(coef_home[1])%>% round(2), ", 95\\% CI ", ci_home)`, p = `r coef_home[3] %>% format.pval(eps=.001)`  ).
For readmission in 30 days, impaired cognition was `r if_else(coef_readmit[3] < .05,"".  "not ")` a significant predictor (odds ratio `r paste0(exp(coef_readmit[1])%>% round(2), ", 95\\% CI ", ci_readmit)`, p = `r coef_readmit[3] %>% format.pval(eps=.001)`  ).
Similarly for near-term death, impaired cognition was `r if_else(coef_death[3] < .05,"".  "not ")` a significant predictor (odds ratio `r paste0(exp(coef_death[1])%>% round(2), ", 95\\% CI ", ci_death)`, p = `r coef_death[3] %>% format.pval(eps=.001)`  ).
Similarly, impaired cognition was `r if_else(coef_los[3] < .05,"".  "not ")` a significant predictor of length of stay (duration ratio `r paste0(exp(coef_los[1])%>% round(2), ", 95\\% CI ", ci_los)`, p = `r coef_los[3] %>% format.pval(eps=.001)`  ).

We found no evidence of heterogeneity by type of surgery, although power for this question is limited by small sample sizes in some surgery types. A forest plot is presented in

```{r figone, echo=FALSE, messages=FALSE, results='asis'}
