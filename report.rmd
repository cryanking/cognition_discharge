...
export LSF_DOCKER_VOLUMES='/storage1/fs1/christopherking/Active/lavanya/cognition_check/:/research/ /home/christopherking/cognition_discharge:/code' bsub -G 'compute-christopherking' -n 2 -R 'rusage[mem=32GB]' -M 32GB -q general -a 'docker(cryanking/cognitioncheck:1.1)' R --args commit=$(git --git-dir /code/.git rev-parse --verify HEAD) -e 'rmarkdown::render("/code/report.rmd")'
...
hosp_proc %>% summarize(across(starts_with("SType") , sum, na.rm=TRUE ) ) %>% t %>% as_tibble(rownames="SurgeryType") %>% swap_pretty_names %>% set_colnames(c('Surgery Type', 'N')) %>% arrange(N) %>% kbl( caption="\\label{tab:descType}Number of Procedures by Type")
```
