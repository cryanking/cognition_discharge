...
---
<!-- docker run -v '/mnt/ris/lavanya/cognition_check/:/research/'  -v '/home/christopherking/gitdirs/cognition_discharge/:/code/' cryanking/cognitioncheck:1.1 R -e 'rmarkdown::render("/code/report.rmd")' --args commit=$(git --git-dir /code/.git rev-parse --verify HEAD)

LSF_DOCKER_VOLUMES='/storage1/fs1/christopherking/Active/lavanya/cognition_check/:/research/ /home/christopherking/cognition_discharge:/code' bsub -G 'compute-christopherking' -n 2 -R 'rusage[mem=32GB] span[hosts=1]' -M 32GB -q general -a 'docker(cryanking/cognitioncheck:1.1)' R  -e 'rmarkdown::render("/code/report.rmd")' --args commit=$(git --git-dir ~/cognition_discharge/.git rev-parse --verify HEAD)
...
outcome_factors<- c("dc_home" , "readmit_30", "death", "ICU" )
outcome_con <- "LoS"
con_vars <- c( 'Age' , 'BMI')
...
We found that impaired cognition was `r if_else(coef_home[4] < .05, "" ,  "not ")` significantly associated with discharge to other than home (odds ratio `r paste0(exp(coef_home[2])%>% round(2) %>% sprintf(fmt="%.2f"), ", 95\\% CI ", ci_home)`, p = `r coef_home[4] %>% format.pval(eps=.001, digits=3)`).
...
knitr::include_graphics("/research/forest_home1.png")
```
