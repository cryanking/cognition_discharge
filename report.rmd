...
5. Some of the exploratory outcomes are not well recorded: PNA, UTI, arrythmia being new
...
outcome_factors<- c("dc_home" , "readmit_30", "death" )
outcome_con <- "LoS"
con_vars <- c( 'Age' , 'BMI')
...
hosp_proc %>% summarize(across(starts_with("SType") , sum, na.rm=TRUE ) ) %>% t %>% as_tibble(rownames="SurgeryType") %>% swap_pretty_names %>% set_colnames(c('Surgery Type', 'N')) %>% kbl( caption="\\label{tab:descType}Number of Procedures by Type")
```
